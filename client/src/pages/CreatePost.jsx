...
const CreatePost = () => {
  ...
  const [generationImage, setGenerationImage] = useState(false);
  ...
  async function generateImage() {
    if (form.prompt) {
      try {
        ...
        const response = await fetch('https://dell-e-3ebg.onrender.com/api/v1/dalle', {
          method: 'POST',
          headers: {
  ...
  async function handleSubmit(e) {
    ...
    if (form.prompt && form.photo) {
      setLoading(true);
      try {
        const response = await fetch('https://dell-e-3ebg.onrender.com/api/v1/post', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  return (
    <section className="font-extrabold text-[#222328] text-[32px]">
      <div>
        <h1 className="font-extrabold text-[#222328] text-[32px]">Create</h1>
        <p className="mt-2 text-[#666e75] text-[14px] max-w[500px]">Create imaginative and visually
          stunning images through by DALL-E AI and share them with the community
        </p>
      </div>
      <form className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
        ...
        <div className="mt-5 flex gap-5">
          ...
          >
            {generationImage ? 'Generating...' : 'Generate'}
          </button>
        </div>
        <div className="mt-10">
